...
const expedition = JSON.parse(read('assets/expedition/expedition-catalog.json'));
...
assert.match(forge, /Isolated Forge components/);
assert.match(forge, /building part—not a finished discovery/);
...
assert.match(forge, /id="componentCategory"/);
assert.match(forge, /id="componentFamily"/);
assert.match(forge, /id="componentSlot"/);
assert.match(forge, /id="componentPart"/);
...
assert.match(forge, /Wonder Forge component preview — not a complete discovery/);
assert.match(expeditionScript, /deterministic_category_or_confirmed_family_pool/);
assert.match(database, /approvedUrl \|\| representativeUrl \|\| archetype\.url/);
assert.equal(expedition.counts.catalog_representatives, 186);
assert.equal(expedition.counts.forge_components, 166);
assert.equal(expedition.counts.compatibility_groups, 102);
assert.equal(expedition.policy.approved_exact_image_wins, true);
assert.equal(expedition.policy.components_are_not_discoveries, true);
