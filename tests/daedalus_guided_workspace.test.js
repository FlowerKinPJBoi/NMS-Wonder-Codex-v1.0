...
assert.match(page, /id="guidedMessageImageInput"/);
assert.match(page, /Support Daedalus compute/);
assert.match(page, /src="preview\.js\?v=1\.31\.5"/);
...
assert.match(guided, /const startingReferences = guided\.sessionId \? \[\] : guided\.referenceFiles/);
assert.match(guided, /const messageFiles = \[\.\.\.guided\.messageFiles\]/);
assert.match(guided, /clipboardData\?\.items/);
assert.match(guided, /attached to this pass only/);
assert.match(guided, /clearMessageImages\(\)/);
assert.match(guided, /onGenerationStatus\?\.\(configureSupport\)/);
...
assert.match(styles, /#guidedPreviewEmpty\[hidden\]/);
assert.match(styles, /\.guided-message-images\[hidden\]/);
