(() => {
  ...
  function renderImages(images) {
    ...
    const show = (image) => {
      ...
      const deliveryUrl = `${image.url}${image.url.includes('?') ? '&' : '?'}display=133`;
    ...
    $('#recordThumbnails').innerHTML = approved.map((image) => `<button class="record-thumbnail" type="button" data-id="${escapeHtml(image.id)}"><img src="${escapeHtml(image.url)}${image.url.includes('?') ? '&' : '?'}display=133" alt="${escapeHtml(image.role.replaceAll('_',' '))}"></button>`).join('');
