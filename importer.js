...
const fileInput=$('#fileInput'),chooseButton=$('#chooseButton'),analyzeButton=$('#analyzeButton'),dropZone=$('#dropZone');
chooseButton.onclick=(event)=>{event.stopPropagation();fileInput.click()};dropZone.addEventListener('click',(event)=>{if(event.target!==chooseButton)fileInput.click()});fileInput.onchange=()=>setFile(fileInput.files[0]);
...
function setFile(file){if(!file)return;selectedFile=file;$('#fileName').textContent=`${file.name} — ${(file.size/1024/1024).toFixed(1)} MB`;analyzeButton.disabled=false;}
...
function updateContributorDisplay() {
  ...
  target.innerHTML = name
    ? `Contributor: <strong>${escapeHtml(name)}</strong><br>Your normalized records will enter the pending review queue.`
