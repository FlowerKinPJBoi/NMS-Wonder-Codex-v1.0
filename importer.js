...
function download(name,content,type){const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([content],{type}));a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);window.WonderAnalytics?.track('download',{download_type:(name.split('.').pop()||'file').toLowerCase()})}
...
function render(){$('#results').hidden=false;$('#submitDatabase').disabled=!(report.discoveries.length||report.matches.length);$('#submitDatabase').textContent='Submit to Wonder Database';$('#saveTitle').textContent=report.saveName;const s=report.summary;const stats=[['Pets',s.pets],['Discoveries',s.discoveries],['Exact pet matches',s.matches],['Generation records',s.generations],['Animals',s.Animal],['Flora',s.Flora],['Minerals',s.Mineral],['Unmatched pets',s.unmatchedPets]];$('#statGrid').innerHTML=stats.map(([a,b])=>`<div class="stat"><strong>${b.toLocaleString()}</strong><span>${a}</span></div>`).join('');renderTable();$('#results').scrollIntoView({behavior:'smooth'});window.WonderAnalytics?.track('import_analyzed',{result_count:Number(s.discoveries||0)})}
...
async function submitToDatabase() {
  ...
  try {
    ...
    window.WonderAnalytics?.track('import_submitted', {result_count:Number(queued.discoveries || 0), public_attribution:Boolean(result.public_attribution)});
  } catch (error) {
    status.classList.add('error');
