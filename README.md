# Wonder Codex v1.32.1

Production source for the public Wonder Codex website and API at
`wondercodex.com`.

v1.32.1 adds Pegasus's in-game friend code to the authenticated dispatch panel so the requesting Admin or Tester can add the dedicated host while it prepares. v1.32.0 introduced the first Pegasus Live dispatch path: a dedicated WonderCodex worker claims short-lived, server-validated routes through a separate secret, reports auditable progress, and never receives private requester friend-code data until it owns a valid lease.
...
production corpus version, and makes that lesson available to ranked semantic
and structural retrieval. Reviewers can disable or restore a lesson with an
