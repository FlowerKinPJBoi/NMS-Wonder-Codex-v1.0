...
from ..services.catalog import display_name, serialize_discovery, wc_id
from ..services.locations import GALAXY_NAMES, decode_portal_coordinates
...
def list_map_points(
    ...
    if lane in {"wonders", "all"}:
        ...
        for row in rows:
            public = serialize_discovery(row)
            if public.get("galaxy_number") != galaxy_number or not public.get("has_travel_address"):
                continue
            if location_quality == "all" and public.get("travel_status") not in {"verified", "derived"}:
                continue
            if location_quality != "all" and public.get("travel_status") != location_quality:
                continue
            ...
            if not _matches_text([record_id, name, row.discovery_type, family_id, family_label], cleaned_query):
                continue
            coordinates = decode_portal_coordinates(public.get("portal_glyphs"))
            if not coordinates:
                continue
            available += 1
            if len(points) >= limit:
            ...
            points.append({
                ...
                "portal_glyphs": public["portal_glyphs"],
                "x": coordinates["x"],
                "y": coordinates["y"],
                "z": coordinates["z"],
                "travel_status": public["travel_status"],
